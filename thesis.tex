...
$RS$ in FreSh are provided in Section~\ref{}.
...
are assigned to the index workers—threads dedicated to the creation of the index while 
...
traverse objects (see Figure \ref{alg:DynamiciSAXTraverse}). The procedure is no longer a
...
the non-overlapping property \ref{def:traverse}, as it can now only be applied to the
...
data (see Algorithm \ref{alg:DynamicISAXTraverse:reuse}). This is necessary because in a
...
(see Algorithm line \ref{alg:DynamicISAXTraverse:reuse}).
% 
Additionally, to support dynamic processing, both types of workers need to perform certain
preparations. In lines \ref{alg:DynamiciSAXTraverse:updates-to-process-Start}-
\ref{alg:DynamiciSAXTraverse:updates-to-process-End}, index workers repeatedly execute the
...
as shown in line \ref{alg:DynamiciSAXTraverse:no-update-yet}.
