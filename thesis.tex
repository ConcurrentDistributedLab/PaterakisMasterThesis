...
\newcommand{\csdchair}{Grigorios Tsagkatakis}
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \Function{TreeInsert}{$\mathit{data}, \mathit{isHelper}, \mathit{model}, \mathit{batchID}$}
            ...
            \While{\True} \label{alg:tree:while_dynamic}
                ...
                \If{$\mathit{pos < M}$} \label{alg:tree:pos-in-D_dynamic}
                    ...
                    \EndIf
                    \If{\br{ $\mathit{Elements.GetIndex} \leq M$ \textbf{and} $\mathit{model} = \mathit{Linearizable}$}}
                    ...
                    \If{\br{$\mathit{model == Linearizable}$}} \label{alg:tree:store-timestamp-start}
                    ...
                    \ElsIf{\tl{$\mathit{model == Timestamps}$}}
...
\begin{algorithm}[t]
    \footnotesize
    \begin{algorithmic}[1]
        \Procedure{SplitLeaf}{$\mathit{leaf}$, $\mathit{prt}$, $\mathit{expeditive}$, $\mathit{model}$}
            ...
            \If{$\mathit{expeditive = \False}$} \label{alg:tree:em:Announce-scan-dynamic}
                \For{$i \in \{0, \ldots, n-1\}$ \textbf{with} $\mathit{leaf \rightarrow Announce[i].data} \neq \bot$} \label{alg:tree:Announce-scan-dynamic}
                    \State $\mathit{ldata} $ := $ \mathit{leaf \rightarrow Announce[i].data}$ \label{alg:tree:announce-scan:D-data}
                    \If{$\mathit{leaf \rightarrow Announce[i].position} \neq \bot$} \label{alg:tree:announce-scan:pos-not-bot-dynamic}
                        \State $\mathit{pos} $ := $ \mathit{leaf \rightarrow Announce[i].position}$ \label{alg:tree:announce-scan:D-position}
                        \State $\mathit{leaf \rightarrow D[\mathit{pos}].data} $ := $ \mathit{ldata}$ \label{alg:tree:announce-scan:store-in-D-dynamic}
                        
                        \If{\br{$\mathit{model == Linearizable}$}}
                        ...
                        \EndIf
                    \Else \label{alg:tree:add-splitbuffer-D}
                        \If{\br{$\mathit{model == Linearizable}$}} 
            ...
            \For{\textbf{each} element in $\mathit{leaf \rightarrow D[i]} \cup \mathit{splitBuffer}$}
                \If{\bl{$\mathit{element.seq == \bot\ \And\ element \in D}$}} \label{alg:split-dynamic-belong-D-start}
                    \If{\br{$\mathit{model == Linearizable}$}}
                        \If{\br{$\mathit{leaf \rightarrow D[\mathit{pos}].seq == MARKED}$}}
                            \State \br{$\mathit{CAS(\&leaf \rightarrow D[\mathit{pos}].seq, MARKED, \bot)}$}
                        \Else
                    ...
                    \ElsIf{\tl{$\mathit{model == Timestamps}$}}
                        ...
                        \State \tl{$\mathit{leaf \rightarrow D[\mathit{pos}].seq} $ := $ \mathit{tmstamp}$}
                    \EndIf
                \EndIf \label{alg:split-dynamic-belong-D-end}
...
\begin{algorithm}[t]
    ...
    \begin{algorithmic}[1]
        ...
        \Procedure{DeleteMin}{Boolean $\mathit{isHelper}$, int $\mathit{CurrentUpdate}$, int $\mathit{queryTS}$} \textbf{returns} $\mathit{Data}$
            ...
            \EndIf
            \If{\br{$\mathit{model == Linearizable}$}}
                \If{\br{$\mathit{SA[pos].data.seq == MARKED}$}}
                    \State \br{\Return $\bot$}
                \EndIf            
                \If{\br{$\mathit{SA[pos].data.seq == \bot}$}}
                    \State \br{int $\mathit{currSeq := GlobalSeq}$}
                    \State \br{$\mathit{CAS(\&SA[pos].data.seq, \bot, currSeq)}$} \label{alg:pq:CAS:Dynamic}
                \EndIf
            \ElsIf{\tl{$\mathit{model == Timestamps}$}}
                \If{\tl{$\mathit{SA[pos].data.seq > queryTS}$}}
                    \State \tl{\Return $\bot$}
                \EndIf 
            \EndIf
...
\noindent{\bf Timestamp  Based Model.} This version of DFreSh uses timestamps to 
synchoronize index and query workers, which is commonly used in the domain of 
databases. This version does not allow queries with a timestamp larger than or equal
to the ongoing batch run concurrently. For this reason, syncrhonization is not required
for this model in the algorithms provided below.

\noindent{\bf Linearizable Based Model.} This version of DFreSh is inspred by the concurrent
computing world and uses \textit{linearizability} to ensure correctness between concurrent
batch insertions and queries.
...
need to wait for each other as long as they have available work. 
 
To ensure correctness and syncronize index construction and query answering workers
this model uses sequence numbers. 
Specifically, we maintain a global shared counter, \textit{GlobalSeq}, which is accessible to
both index and query workers. This counter serves as the sequence number,
representing either a data series or a query, depending on whether its value has been read
by index or query workers.
Before answering a query, query workers attempt to atomically increment the value
of \textit{GlobalSeq} by one.
The index workers also read the \textit{GlobalSeq} during each insert and assign 
a sequence number to the data series they insert in the tree index.
A query must be able to see all data series inserted
into the index tree with a sequence number smaller than or equal to its own.

\noindent{\textbf{Tree Insertion.}}
...
The elements of array \( D \) contained in each leaf now store 
the sequece number of each data series in addition 
to the iSAX summary and the pointer to the actual data series. 
Despite these differences, the core insertion procedure remains
similar to what was described in Algorithm~\ref{alg:tree}.  
...
stored as described in Section~\ref{sec:buffers_and_tree}. However, for an element to be considered valid, it must first be assigned
...
(Algorithm~\ref{alg:pq-dynamic} line~\ref{alg:pq:CAS:Dynamic}). Since both index and query
workers can assign sequence numbers, the CAS operation ensures correctness while the
preceding conditional check optimizes performance by avoiding unnecessary
failed CAS attempts.

\noindent{\textbf{Split Operation.}}
If the leaf is full, the index worker attempts to perform a split.  
During the split operation (Algorithm~\ref{alg:tree-split-dynamic}), the index worker
first examines the announce array (line~\ref{alg:tree:Announce-scan-dynamic})
...
(lines~\ref{alg:tree:announce-scan:D-data} and~\ref{alg:tree:announce-scan:D-position}),
the thread performing the split rewrites the data into that position (line~\ref{alg:tree:announce-scan:store-in-D-dynamic}),
...
(lines~\ref{alg:tree:add-splitbuffer-D}-\ref{alg:tree:announce-scan:data-copy-dynamic})
as it did in FreSh.  
...
if necessary (Algorithm ~\ref{alg:tree-split-dynamic} lines~\ref{alg:split-dynamic-belong-D-start}-\ref{alg:split-dynamic-belong-D-end}).
This step is crucial because the leaf remains connected to the
...
of the counter that is increased using the \textit{NextIndex}. If a split is occurring, the thread marks its position
